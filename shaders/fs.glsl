...
uniform mat4 lightPos0;			// light position
uniform vec3 lightColor0;		// light color

uniform mat4 lightPos1;			// light position
uniform vec3 lightColor1;		// light color

uniform mat4 lightPos2;			// light position
uniform vec3 lightColor2;		// light color

uniform mat4 lightPos3;			// light position
...
void main()
{
	...
	vec4 specColor1 = vec4(lightColor1, 1) * pow(max(dot(R1, L1), 0.0), 0.3*250) * (materialColor1, 1);
	...
	vec4 specColor2 = vec4(lightColor2, 1) * pow(max(dot(R2, L2), 0.0), 0.3*250) * (materialColor2, 1);
	...
	vec4 specColor3 = vec4(lightColor3, 1) * pow(max(dot(R3, L3), 0.0), 0.3*250) * (materialColor3, 1);
